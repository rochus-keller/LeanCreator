...
LocalHelpManager::HelpData LocalHelpManager::helpData(const QUrl &url)
{
    ...
    if (data.resolvedUrl.isValid()) {
        const QString path = data.resolvedUrl.toString();
#if 1
        // meanwhile I fixed the qcssscanner and QCss::Parser so it can handle NBSP and *=
        // but the offline-simple.css from Qt 5.6.3 still seems to work better than the one in the qch
        if( path.startsWith("qthelp://") && path.endsWith("style/offline.css") )
        {
            // replace the offline.css included in the qch by another one which works better.
            // originally to avoid the incompatibility of this css with this QCss::Parser
            QFile in(":/help/offline.css");
            in.open(QIODevice::ReadOnly);
            data.data = in.readAll();
        }else
#endif
...
#if 0
        QFile f( QCoreApplication::applicationDirPath() + "/" + data.resolvedUrl.fileName() );
