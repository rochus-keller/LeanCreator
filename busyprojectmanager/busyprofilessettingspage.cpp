...
#include "ui_busyprofilessettingswidget.h"

#include "custombusypropertiesdialog.h"
#include "busyconstants.h"
...
class BusyProfilesSettingsWidget : public QWidget
{
    Q_OBJECT
public:
    BusyProfilesSettingsWidget(QWidget *parent = 0);
...
private:
    ...
    Ui::BusyProfilesSettingsWidget m_ui;
    busy::SettingsModel m_model;
...
BusyProfilesSettingsPage::BusyProfilesSettingsPage(QObject *parent)
    : Core::IOptionsPage(parent), m_widget(0)
{
    setId("AA.BusyProfiles");
    setDisplayName(QCoreApplication::translate("BusyProjectManager", "Profiles"));
    setCategory(Constants::BUSY_SETTINGS_CATEGORY);
    setDisplayCategory(QCoreApplication::translate("BusyProjectManager",
                                                   Constants::BUSY_SETTINGS_TR_CATEGORY));
    setCategoryIcon(QLatin1String(Constants::BUSY_SETTINGS_CATEGORY_ICON));
}

QWidget *BusyProfilesSettingsPage::widget()
{
    if (!m_widget)
        m_widget = new BusyProfilesSettingsWidget;
    return m_widget;
}

void BusyProfilesSettingsPage::apply()
{
    if (m_widget)
        m_widget->applyCustomProperties();
}

void BusyProfilesSettingsPage::finish()
{
    delete m_widget;
    m_widget = 0;
}
...
BusyProfilesSettingsWidget::BusyProfilesSettingsWidget(QWidget *parent)
    : QWidget(parent)
    ...
    , m_applyingProperties(false)
{
    ...
    connect(ProjectExplorer::KitManager::instance(), &ProjectExplorer::KitManager::kitsChanged,
            this, &BusyProfilesSettingsWidget::refreshKitsList);
...
void BusyProfilesSettingsWidget::applyCustomProperties()
{
    ...
    for (CustomProperties::ConstIterator it = m_customProperties.constBegin();
        ...
        kit->setValue(Core::Id(Constants::BUSY_PROPERTIES_KEY_FOR_KITS), it.value());
    }
    m_applyingProperties = false;
    m_model.reload();
...
void BusyProfilesSettingsWidget::refreshKitsList()
{
...
void BusyProfilesSettingsWidget::displayCurrentProfile()
{
    m_ui.propertiesView->setModel(0);
    ...
    const QString profileName = BusyManager::instance()->profileForKit(kit);
...
void BusyProfilesSettingsWidget::editProfile()
{
    QTC_ASSERT(m_ui.kitsComboBox->currentIndex() != -1, return);

    const Core::Id kitId = Core::Id::fromSetting(m_ui.kitsComboBox->currentData());
    CustomBusyPropertiesDialog dlg(m_customProperties.value(kitId), this);
...
void BusyProfilesSettingsWidget::setupCustomProperties(const ProjectExplorer::Kit *kit)
{
    const QVariantMap &properties
            = kit->value(Core::Id(Constants::BUSY_PROPERTIES_KEY_FOR_KITS)).toMap();
    m_customProperties.insert(kit->id(), properties);
}

void BusyProfilesSettingsWidget::mergeCustomPropertiesIntoModel()
{
    ...
    for (CustomProperties::ConstIterator it = m_customProperties.constBegin();
        ...
        const QString keyPrefix = QLatin1String("profiles.")
                + BusyManager::instance()->profileForKit(kit) + QLatin1Char('.');
...
#include "busyprofilessettingspage.moc"
