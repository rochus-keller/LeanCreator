                                                ...
                                                const QVariantMap &defaultData) const
{
    ...
    const QVariantMap customProperties = k->value(Core::Id(BUSY_PROPERTIES_KEY_FOR_KITS)).toMap();
                                                             ...
                                                             const QVariantMap &defaultData) const
{
    ...
    if (ProjectExplorer::SysRootKitInformation::hasSysRoot(k))
        data.insert(QLatin1String(BUSY_SYSROOT), sysroot);
    ...
    if (targetAbi.architecture() != ProjectExplorer::Abi::UnknownArchitecture) {
        ...
        // data.insert(QLatin1String(BUSY_ARCHITECTURE), busy::canonicalArchitecture(architecture));
    }

    QStringList targetOS = targetOSList(targetAbi, k);
    if (!targetOS.isEmpty())
        data.insert(QLatin1String(BUSY_TARGETOS), targetOS);

    QStringList toolchain = toolchainList(tc);
    if (!toolchain.isEmpty())
        data.insert(QLatin1String(BUSY_TOOLCHAIN), toolchain);

    if (targetAbi.os() == ProjectExplorer::Abi::MacOS) {
        // Set Xcode SDK name and version - required by Busy if a sysroot is present
