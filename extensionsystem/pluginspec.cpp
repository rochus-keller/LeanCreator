...
bool PluginSpecPrivate::read(const QString &fileName)
{
    ...
    if (loader.fileName().isEmpty()) {
        qCDebug(pluginLog) << "Cannot open file" << filePath;
#else
    {
#endif
        qCDebug(pluginLog) << "Loading plugins from" << filePath << "not supported by this version";
        return false;
    }
...
bool PluginSpecPrivate::loadLibrary()
{
    ...
    if( candidate )
    {
        IPlugin *pluginObject = qobject_cast<IPlugin*>(candidate);
        if (!pluginObject) {
            hasError = true;
...
#else
    hasError = true;
    errorString = QCoreApplication::translate("PluginSpec", "Plugins cannot be loaded by this version");
    return false;
#endif
}
