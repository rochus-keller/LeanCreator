...
#include <core/removefiledialog.h>
...
bool ProjectExplorerPlugin::coreAboutToClose()
{
    if (BuildManager::isBuilding()) {
        ...
        box.setDefaultButton(cancelClose);
        box.setWindowTitle(tr("Close LeanCreator?"));
        box.setText(tr("A project is currently being built."));
        box.setInformativeText(tr("Do you want to cancel the build process and close LeanCreator anyway?"));
        box.exec();
        if (box.clickedButton() != closeAnyway)
            return false;
    }
